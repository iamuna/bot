...
cd /d "%~dp0"
title Terminal 3 v2.4.2 Frozen OOS Battery
...
echo.
echo Starting the frozen v2.4.2 out-of-sample battery...
echo This window stays open on success OR failure.
echo.
"%PY%" -u launcher_oos_v242.py %*
set "RC=%ERRORLEVEL%"
goto :finish
...
:finish
echo.
if "%RC%"=="0" echo OOS launcher finished successfully.
if not "%RC%"=="0" echo OOS launcher stopped with error code %RC%.
echo.
