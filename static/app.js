(() => {
  ...
  let state=null, selectedTF='1h', chartLimit=120, chartPayload=null, settingsSeeded=false, matrixGroup='all';
  ...
  function renderSettings(r){if(!r)return;if(!settingsSeeded){$('signalMode').value=r.signal_mode||'Aggressive';settingsSeeded=true}setRange('riskPct',r.risk_pct,'%');setRange('leverage',r.leverage,'×');setRange('minQuality',r.min_quality,'%');setRange('minConfluence',r.min_confluence,'%');setRange('portfolioRisk',r.max_total_open_risk_pct,'%');setRange('targetR',r.tp_r_multiple,'R')}
  function render(s){
    ...
    if(s.environment==='live')$('warning').textContent='REAL-MONEY MODE. No daily trade-count limit or cooldown. BloFin hard-caps Multi-Position mode at 10 simultaneous positions per instrument.';
  ...
  async function applyTimeframes(){const enabled=new Set((state.runtime||{}).enabled_timeframes||[]);document.querySelectorAll('.tf-toggle').forEach(x=>x.checked?enabled.add(x.dataset.toggle):enabled.delete(x.dataset.toggle));try{await req('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled_timeframes:TF_ORDER.filter(tf=>enabled.has(tf))})});poll()}catch(e){alert(e.message)}}
  ...
  async function loadChart(){try{chartPayload=await req(`/api/chart?tf=${encodeURIComponent(selectedTF)}&limit=${chartLimit}`);const xs=chartPayload.candles||[],a=chartPayload.analysis||{},last=xs[xs.length-1];let closeTxt='';if(last&&last.ct){const sec=Math.max(0,Math.ceil((+last.ct-Date.now())/1000));closeTxt=last.closed?' · closed':' · next close '+Math.floor(sec/60)+'m '+(sec%60)+'s'}$('chartLabel').textContent=`${selectedTF} · ${DERIVED.has(selectedTF)?'constructed':'native'} · ${xs.length} candles · score ${signed(a.score)}${closeTxt}`;$('zoomLabel').textContent=`${chartLimit} candles`;drawChart()}catch(e){$('chartReadout').textContent=e.message}}
  function drawChart(){
