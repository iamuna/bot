...
from backtest_v24 import Backtester, Candidate, load_1m_csv, save_report, resolve_time_range
...
class GuardedBacktester(Backtester):
    ...
    def _open_candidate(self, c: Candidate, bar):
        ...
        if net_r < self.guard.min_net_target_r:
            ...
            return
        profit_mult, _giveback = self._profit_guard_multiplier(float(bar['o']))
...
def main():
    ...
    ap.add_argument('--fee-bps', type=float, default=6.0, help='per-side fee assumption')
    ...
    r = GuardedBacktester(base, a.equity, a.fee_bps, a.slippage_bps, guard).run()
